...
const StoryCard = ({
  ...
  story: SuccessStory;
  index: number;
}) => <motion.div key={story.id} initial={{
  opacity: 0,
  y: 20
}} animate={{
  opacity: 1,
  ...
  useEffect(() => {
    const fetchStories = async () => {
      const {
        data,
        error
      } = await supabase.from('success_stories').select('*').order('created_at', {
        ascending: false
      });
      if (error) {
    ...
    fetchStories();
    const channel = supabase.channel('success-stories-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
